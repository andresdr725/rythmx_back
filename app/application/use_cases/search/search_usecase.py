...
class SearchUseCase:
    ...
    def execute(self, query: SearchInput) -> List[SearchOutput]:
        ...
        if query.type == Category.SONGS:
            response: List[SongResponse] = self.services.search_songs(query)

        elif query.type == Category.ALBUMS:
            response: List[AlbumResponse] = self.services.search_albums(query)
        
        elif query.type == Category.PLAYLISTS:
            response: List[PlaylistResponse] = self.services.search_playlists(query)
